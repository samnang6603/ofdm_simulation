function y = rf_downconvert_impairment_IQimb_mitigate(x,RF)
% IQ impairment mitigation
%
%
%
%{
    % With pilot extension y = rf_downconvert_impairment_IQimb_mitigate(x,tx_pilot,pilot_index)
    %
    %
...
% Adaptive filter
L = length(x);
compSig = complex(zeros(L,1),zeros(L,1));
w = complex(0,0);
mu = 1e-5;

for n = 1:RF.IMPAIRMENT.IQImbalance.Mitigation.Iteration % needs more iteration
    for m = 1:L
        compSig(m) = x(m) + w*conj(x(m));
